...
from beancount.core.data import Transaction, Amount, Decimal, Posting, Balance
...
class CMBCredit():
  ...
  def change_currency (self, currency):
    ...
    if currency not in trade_area_list:
      print('Unknown trade area: ' + currency + ', please append it to ' + __file__)
      return currency
  ...
  def parse (self):
    ...
    for band in bands:
      ...
      entry = Transaction(meta, time, flag, payee, description, data.EMPTY_SET, data.EMPTY_SET, [])
      ...
      if real_currency == trade_currency:
        ...
        real_amount = Amount(Decimal(abs(round(float(real_price), 2))) / Decimal(abs(round(float(trade_price), 2))), real_currency)
